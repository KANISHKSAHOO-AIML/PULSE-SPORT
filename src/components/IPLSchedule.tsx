...
import { IPL_2026_SCHEDULE, IPLMatch, getDynamicSchedule } from "@/lib/ipl2026Schedule";
  ...
  return (
    <div className="grid gap-4 max-w-4xl mx-auto">
      {schedule.map((match, i) => {
        ...
        return (
          <Link key={match.matchNo} href={`/matches/${match.matchNo}`} className="block">
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: i * 0.05, duration: 0.3 }}
              className={`flex flex-col md:flex-row items-center justify-between p-4 md:p-5 rounded-2xl border ${isLive ? 'border-red-500/30 bg-red-500/5' : 'border-white/5 bg-[#0d0d1a]/80 backdrop-blur-xl'} hover:border-white/10 hover:bg-white/[0.02] transition-all cursor-pointer`}
            >
              {/* Match Info */}
