...
const PLAYER_DATA: Record<string, Record<string, { name: string; stats: number[] }>> = {
  cricket: {
...
const STAT_LABELS: Record<string, string[]> = {
  cricket: ["Runs", "Wickets", "Strike Rate", "Catches", "Consistency"],
  football: ["Goals", "Assists", "Dribbling", "Tackling", "Pace"],
};

function getPlayerForTeam(sport: string, team: string) {
  const sportData = PLAYER_DATA[sport] || {};
  return sportData[team] || { name: "Star Player", stats: [75, 70, 72, 65, 68] };
}
...
export default function PlayerComparison({ sport, teamA, teamB }: PlayerComparisonProps) {
  ...
  const playerA = getPlayerForTeam(sport, teamA);
  const playerB = getPlayerForTeam(sport, teamB);
